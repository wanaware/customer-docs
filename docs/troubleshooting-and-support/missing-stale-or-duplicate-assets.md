...
5. Confirm that the latest provider crawl or bulk import finished.
...
| Newly connected resource is missing | Crawl or ingestion is still processing | Wait for completion and refresh; do not run repeated overlapping crawls. |
| Asset details look old | Source has not been crawled since the change | Verify the integration, run one approved crawl, and allow processing to finish. |
